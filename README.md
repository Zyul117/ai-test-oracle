...
软件测试课上学到 Oracle Problem——测试里最大的难点不是"生成测试数据"，而是"判断结果对不对"。

举个栗子：一个接口返回了 `balance: -500`，断言 `status_code == 200` 是通过的，但余额为负显然是个 Bug。传统断言只能检查你提前想到的问题，这种隐性 Bug 很难被发现。

我试着用 LLM 当"审查员"，让 AI 来判断响应是不是真的正确。做下来发现有用，但也有不少局限。
